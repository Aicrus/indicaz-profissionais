...
class IndicazProfissionaisSupabaseUser extends BaseAuthUser {
  IndicazProfissionaisSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> indicazProfissionaisSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IndicazProfissionaisSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
